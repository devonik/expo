...
describe('asyncRequireModule', () => {
  ...
  beforeEach(() => {
    ...
    // Evaluate a handwritten source copy in a scope where `require` is our mock.
    // Keep asyncRequireImpl, maybeLoadBundle, and any copied worker resolution
    // behavior synchronized with production when those branches change.
    ...
    const moduleFn = new Function(
      ...
      function asyncRequireImpl(moduleID, paths, moduleName) {
        ...
        if (process.env.EXPO_OS === 'web') {
          var payload = paths && paths[String(moduleID)];
          if (Array.isArray(payload)) {
            var loader = globalThis[(__METRO_GLOBAL_PREFIX__ || '') + '__loadBundleAsync'];
            if (loader && loader.isReady && loader.isReady(payload)) return importAll();
            var loading = maybeLoadBundle(moduleID, paths);
